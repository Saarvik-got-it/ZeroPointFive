...
import { ChevronRight, Play, Star, Heart } from 'lucide-react';
...
const episodes = [
  {
    ...
    guests: [
      { name: 'Michael Ross', image: 401 },
      { name: 'Emma Wilson', image: 402 }
    ],
    subtitle: 'Resilient Leadership',
    duration: '48 min',
    rating: 4.9,
    category: 'Leadership'
  },
  {
    ...
    guests: [
      { name: 'Priya Singh', image: 403 },
      { name: 'Tom Chen', image: 404 }
    ],
    subtitle: 'Building billion dollar companies',
    duration: '1h 15min',
    rating: 4.8,
    category: 'Startups'
  },
  {
    ...
    guests: [
      { name: 'Tom Anderson', image: 405 },
      { name: 'Maya Johnson', image: 406 }
    ],
  ...
  },
  {
    ...
    guests: [
      { name: 'Lisa Wang', image: 407 },
      { name: 'Ryan Cooper', image: 408 }
    ],
    subtitle: 'What\'s next in technology',
    duration: '1h 2min',
    rating: 4.9,
    category: 'Technology'
  }
          ...
          >
            <div className="bg-gradient-to-br from-[#0D0D0D] via-[#0A0A0A] to-[#050505] backdrop-blur-xl border border-[#FF7A1A]/20 rounded-2xl overflow-hidden hover:border-[#FF7A1A]/70 transition-all duration-700 shadow-2xl hover:shadow-[0_20px_60px_-15px_rgba(255,122,26,0.4)]">
              ...
              <div className="relative h-44 bg-gradient-to-br from-[#1A1A1A] to-[#0D0D0D] flex items-center justify-center overflow-hidden">
                ...
                {/* Overlapping Circles - Different Style */}
                <div className="relative flex items-center justify-center">
                  {/* First Photo - Smaller */}
                  <motion.div
                    className="relative w-24 h-24 rounded-full border-4 border-[#FF7A1A]/50 overflow-hidden shadow-2xl shadow-[#FF7A1A]/20"
                    whileHover={{ scale: 1.2, x: -10, zIndex: 20 }}
                    transition={{ type: "spring", stiffness: 300, damping: 20 }}
                  >
                    <img
                      src={`https://i.pravatar.cc/300?img=${episode.guests[0].image % 70}`}
                      alt={episode.guests[0].name}
                      className="w-full h-full object-cover"
                    />
                    {/* Glow ring */}
                    <div className="absolute inset-0 rounded-full ring-2 ring-inset ring-[#FF7A1A]/30" />
                  </motion.div>

                  {/* Second Photo - Larger and Overlapping */}
                  <motion.div
                    className="relative w-28 h-28 rounded-full border-4 border-[#FF9A3D]/60 overflow-hidden shadow-2xl shadow-[#FF9A3D]/30 -ml-12"
                    whileHover={{ scale: 1.2, x: 10, zIndex: 20 }}
                    transition={{ type: "spring", stiffness: 300, damping: 20 }}
                  >
                    <img
                      src={`https://i.pravatar.cc/300?img=${episode.guests[1].image % 70}`}
                      alt={episode.guests[1].name}
                      className="w-full h-full object-cover"
                    />
                    {/* Shimmer on hover */}
                    <motion.div
                ...
                >
                  <motion.div
                    ...
                    whileTap={{ scale: 0.9 }}
                    className="w-16 h-16 rounded-full bg-gradient-to-br from-[#FF7A1A] to-[#FF9A3D] flex items-center justify-center shadow-2xl shadow-[#FF7A1A]/60"
                  >
              ...
              <div className="p-5">
                ...
                <motion.div
                  ...
                  className="inline-block px-3 py-1.5 bg-gradient-to-r from-[#FF7A1A]/15 to-[#FF9A3D]/15 border border-[#FF7A1A]/40 rounded-full mb-3"
                >
                  <span className="text-[#FF7A1A] text-xs font-bold">{episode.category}</span>
                </motion.div>

                {/* Title */}
                <h3 className="text-[#FAFAF8] font-bold text-lg mb-1 line-clamp-1 leading-tight group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-[#FF7A1A] group-hover:to-[#FF9A3D] group-hover:bg-clip-text transition-all duration-300">
                  {episode.title}
                </h3>

                {/* Subtitle */}
                <p className="text-[#9C9C9C] text-sm mb-4 line-clamp-1">{episode.subtitle}</p>
            ...
            <motion.div
              ...
              className="absolute inset-0 -z-20 bg-gradient-to-br from-[#FFD700]/15 to-[#FF7A1A]/20 rounded-2xl blur-3xl scale-110"
            />
          </motion.div>
        ))}
      </div>
