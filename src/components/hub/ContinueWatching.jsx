...
const episodes = [
  { id: 1, title: 'Mastering AI Ethics', guest: 'Alex Chen', duration: '32 min', viewed: '18 min', progress: 56, category: 'AI' },
  { id: 2, title: 'Product Leadership', guest: 'Sarah Williams', duration: '45 min', viewed: '20 min', progress: 44, category: 'Product' },
  { id: 3, title: 'Startup Fundraising', guest: 'David Park', duration: '55 min', viewed: '30 min', progress: 54, category: 'Startups' },
  { id: 4, title: 'Engineering Culture', guest: 'Maria Garcia', duration: '48 min', viewed: '15 min', progress: 31, category: 'Engineering' }
];
    ...
    <section>
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-3xl font-bold bg-gradient-to-r from-[#FAFAF8] to-[#9C9C9C] bg-clip-text text-transparent">Continue Watching</h2>
            ...
            <div className="bg-gradient-to-br from-[#0D0D0D] via-[#0A0A0A] to-[#050505] backdrop-blur-xl border border-[#FF7A1A]/20 rounded-2xl overflow-hidden hover:border-[#FF7A1A]/70 transition-all duration-700 shadow-2xl hover:shadow-[0_20px_60px_-15px_rgba(255,122,26,0.4)]">
              <div className="relative h-44 overflow-hidden bg-gradient-to-br from-[#1A1A1A] to-[#0D0D0D]">
                <motion.div className="absolute inset-0" whileHover={{ scale: 1.08 }} transition={{ duration: 0.7, ease: 'easeOut' }}>
                  <img src={`https://i.pravatar.cc/400?img=${episode.id + 20}`} alt={episode.guest} className="w-full h-full object-cover" />
                </motion.div>
                ...
                <div className="absolute bottom-0 left-0 right-0 h-1.5 bg-black/70 backdrop-blur-sm">
                  <motion.div className="h-full bg-gradient-to-r from-[#FF7A1A] via-[#FF9A3D] to-[#C9A46A]" initial={{ width: 0 }} animate={{ width: `${episode.progress}%` }} transition={{ duration: 1.2, delay: index * 0.1 + 0.3, ease: 'easeOut' }} />
                </div>
              </div>
              <div className="p-5">
                ...
                <h3 className="text-[#FAFAF8] font-bold text-lg mb-2 leading-tight group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-[#FF7A1A] group-hover:to-[#FF9A3D] group-hover:bg-clip-text transition-all duration-300">{episode.title}</h3>
                <div className="flex items-center justify-between">
                  <p className="text-[#FAFAF8] text-sm font-medium">{episode.guest}</p>
                  <motion.span whileHover={{ scale: 1.05 }} className="px-3 py-1 bg-gradient-to-r from-[#FF7A1A]/15 to-[#FF9A3D]/15 border border-[#FF7A1A]/40 rounded-full text-[#FF7A1A] text-xs font-semibold">{episode.category}</motion.span>
                </div>
              </div>
            </div>
          </motion.div>
